...
void main() {
  ...
  testWidgets("Can toggle testing panel's visibility", (tester) async {
  ...
  testWidgets('Warning boxes are hidden when panel is displayed', (
    tester,
  ) async {
    await tester.pumpWidget(const TestApp(child: TextField()));
    await tester.pumpAndSettle();
  ...
  testWidgets('Can change text scale', (tester) async {
  ...
  testWidgets('Can override locale', (tester) async {
    await tester.pumpWidget(
      MaterialApp(
    ...
    await tester.scrollUntilVisible(find.textContaining('Localization'), 150);

    final tile = find.ancestor(
      of: find.text('Localization'),
      matching: find.byType(MultiValueToggle<Locale>),
    );
    final localeFi = find.descendant(of: tile, matching: find.text('fi_FI'));
    final localeEn = find.descendant(of: tile, matching: find.text('en_US'));
  ...
  testWidgets('Can change target platform', (tester) async {
    late ThemeData themeData;
    await tester.pumpWidget(
      TestApp(
        child: Builder(
  ...
  testWidgets('Can toggle visual density', (tester) async {
    ...
    // Switch to standard
    await tester.tap(_toggleTile<VisualDensity>(itemName, 'standard'));
    await tester.pump();
    expect(themeData.visualDensity, VisualDensity.standard);

    // Switch to comfortable
    await tester.tap(_toggleTile<VisualDensity>(itemName, 'comfortable'));
    await tester.pump();
    expect(themeData.visualDensity, VisualDensity.comfortable);

    // Switch to default value
    await tester.tap(_toggleTile<VisualDensity>(itemName, 'System'));
    await tester.pump();
    expect(themeData.visualDensity, defaultDensity);
  });

  testWidgets('Can toggle bold text', (tester) async {
    late MediaQueryData mediaQuery;
  ...
  testWidgets('Can toggle semantics debugger', (tester) async {
  ...
  testWidgets('Testing tool panel has no accessibility issues', (tester) async {
  ...
  testWidgets('Can move buttons when enableButtonsDrag is true', (
    tester,
  ) async {
    await tester.pumpWidget(
      const TestApp(enableButtonsDrag: true, child: SizedBox()),
    );
    await tester.pump();
...
Future<void> _pan(
  ...
  await tester.fling(buttonFinder, offset, offset.distance * 10);
  await tester.pumpAndSettle();
}
...
Finder _toggleTile<T>(String tileName, String optionName) {
  ...
  return find.descendant(of: tile, matching: find.text(optionName));
}
